...
const MediaCard = ({ item }: MediaProps): JSX.Element => {
  ...
  const handleOpenImages = async (): Promise<void> => {
    ...
    const { sendDeleteMessage } = webSocketChatSrore;
      ...
      openImages: {
        ...
        message: {
          ...
          from_user: {
            ...
            avatar_master_url: item.from_user?.avatar_webp_url || item.from_user?.avatar_small_url || '',
            avatar_small_url: item.from_user?.avatar_webp_url || item.from_user?.avatar_small_url || '',
